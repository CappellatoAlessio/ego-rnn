...
def main_run(dataset, stage, train_data_dir, val_data_dir, stage1_dict, out_dir, seqLen, trainBatchSize,
             valBatchSize, numEpochs, lr1, decay_factor, decay_step, memSize, lossType, variant):
    if dataset == 'gtea61':
        num_classes = 61
    elif dataset == 'gtea71':
        num_classes = 71
    ...
    if stage == 1:
        model_folder = os.path.join('./', out_dir, dataset, 'rgb+cwa', 'stage1')  # Dir for saving models and log files
    else:
        model_folder = os.path.join('./', out_dir, dataset, 'rgb+ms+cwa',
...
def __main__():
    ...
    main_run(dataset, stage, trainDatasetDir, valDatasetDir, stage1Dict, outDir, seqLen, trainBatchSize,
             valBatchSize, numEpochs, lr1, decayRate, stepSize, memSize, lossType, variant)
