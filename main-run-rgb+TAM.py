...
from objectAttentionModelConvLSTMTAM import *
...
def main_run(dataset, stage, train_data_dir, val_data_dir, stage1_dict, out_dir, seqLen, trainBatchSize,
    ...
    model_folder = os.path.join('./', out_dir, dataset, 'rgb+TAM', 'stage'+str(stage))  # Dir for saving models and log files
    # Create the dir
    ...
    for params in model.fc_att1.parameters():
        params.requires_grad = True
        train_params += [params]

    for params in model.fc_att2.parameters():
        params.requires_grad = True
        train_params += [params]
    ...
    model.fc_att1.train(True)
    model.fc_att2.train(True)
    model.cuda()
